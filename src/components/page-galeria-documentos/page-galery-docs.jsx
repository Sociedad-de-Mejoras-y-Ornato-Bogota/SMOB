        ...
        <div className="page_galery_docs">
            ...
            <section style={{ width: "100%", display: "flex", justifyContent: "center" , margin:"1rem"}} className="menu_galery">
                {documents.map((document, index) => {
                    return (
                        <button key={index}  className={`filtro-publicaciones ${selected === index ? "button-selected" : ""}`}   style={{ border: "none", margin: "3rem", color: "#91583B", width:"33.3vw", display:"flex", justifyContent:"center", padding:"1rem" }} onClick={()=>{
                            setSelected(index)
                        }}>
                            {document.title} 
                        </button>
